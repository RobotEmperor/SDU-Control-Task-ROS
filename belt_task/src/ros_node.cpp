...
void RosNode::DisplayToolPathBMsgCallBack(const geometry_msgs::PoseArray::ConstPtr& msg)
{
  ...
  for(int num = 0; num < msg->poses.size(); num ++)
  {
    display_tool_path_b_msg_.poses.push_back(msg->poses[num]);
  }
  ...
  std::cout << "STATUS :: ROBOT B :: Input Paths :: " << check_input_b_paths_ << std::endl;
}
void RosNode::DisplayToolPathMsgCallBack(const geometry_msgs::PoseArray::ConstPtr& msg)
{
  ...
  for(int num = 0; num < msg->poses.size(); num ++)
  {
    display_tool_path_msg_.poses.push_back(msg->poses[num]);
  }
